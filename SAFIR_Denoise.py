...
#
# Copyright (C) 2017 Graeme Ball / Dundee Imaging Facility.
# All rights reserved.
#
...
OMERO script to run Priism ND-SAFIR denoising algorithm (remotely)
...
jobs_filename = 'safir_denoise.jobs'
alive_check_filename = 'lastalive.txt'  # to allow remote cleanup if killed
results_filename = 'safir_denoise.results'
...
TIMEOUT = 18000  # job times out after this many seconds
HOST = 'localhost'  # BlitzGateway .host says 'null' :-[


def run():
    """
    Launch (remote) Priism ND SAFIR denoising job on a list of images.
    ...
    client = script.client(
        "SAFIR_Denoise.py", "ND-SAFIR Denoising",
        ...
        script.String("Patch radius", optional=False,
            description="pixel comparison patch size: 1 best for 3D, 2 for 2D",
            grouping='3', values=PRAD, default=PRAD[0]),

        script.String("Iterations", optional=False,
            description="search radius, e.g. 2=3x3x3x1, 3=5x5x5x3, 4=7x7x7x3",
            grouping='4', values=ITER, default=ITER[2]),

        script.String("Dimensionality", optional=False,
            description="average pixels over dims: xy, xyz, xyzt",
            grouping='5', values=DIMS, default=DIMS[0]),
    ...
    try:
        ...
        job['par.prad'] = client.getInput("Patch radius", unwrap=True)
        job['par.iter'] = client.getInput("Iterations", unwrap=True)
        job['par.dim'] = client.getInput("Dimensionality", unwrap=True)
...
def import_results(results, user, group, sid, conn):
    ...
    for result in results:
        try:
            ...
            if 'fail' in r:
                print "SAFIR denoising failed for imageID=%s: %s" % \
                    (image_id, r['fail'])
            else:
                cli = omero.cli.CLI()
                cli.loadplugins()
                ...
                descrip = "SAFIR denoising result from Image ID: %s" % image_id
